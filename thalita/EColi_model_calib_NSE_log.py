...
def EColi_run(param):
  ...
  ###Listas de E. coli
    ...
    # salvar variaveis em log
    lista_log_csoil_usz_list = []
    lista_log_c_usz_list = []
    lista_log_Rx_usz_list = []
    lista_log_Rx_sz_list = []
    lista_log_csoil_sz_list = []
    lista_log_c_sz_list = []
    lista_log_cpz = []
    ...
    for t in range(len(indice) - 1):
        ...
        # for i in range(0, len(cpz_i)):
        #     if cpz_i[i] == 0:
        #         lista_log_cpz_i = 0
        #     else:
        #         lista_log_cpz_i = log10(abs(cpz_i[i]))
        #     lista_log_cpz.append(lista_log_cpz_i)

        # for i in range(0, len(cpz_i)):
        if cpz_i == 0:
            lista_log_cpz_i = 0
        else:
            lista_log_cpz_i = log10(abs(cpz_i))
        ...
        if m_usz != 0:
            ...
            for l in range(m_usz):
                ...
                ### LOG ###
                if csoil_usz == 0:
                    csoil_i_usz_log.append(csoil_usz)

                else:
                    csoil_i_usz_log.append(log10(abs(csoil_usz)))
                ...
                if Rxi_usz == 0:
                    Rxl_log.append(Rxi_usz * (1 / teta_usz_iplus1) * dt)
                else:
                    Rxl_log.append(log10(abs(Rxi_usz * (1 / teta_usz_iplus1) * dt)))
                ...
                delta_c_usz = ftransport(teta_i_usz, teta_usz_iplus1, cl_usz, csoil_usz, dc, dc_dz, kads1, kdes1, Di_usz,
                                         Vi_usz, Rxi_usz)
                #
                ...
                if ci1 == 0:
                    cl_i1_log.append(ci1)
                else:
                    cl_i1_log.append(log10(abs((ci1))))
            ...
            for j in range(m_sz):
                ...
                ### LOG ###
                if csoil_sz == 0:
                    csoil_i_sz_log.append(csoil_sz)
                else:
                    csoil_i_sz_log.append(log10(abs(csoil_sz)))
                ...
                if Rxi_sz == 0:
                    Rxj_log.append(Rxi_sz * (1 / teta_sz_iplus1) * dt)
                else:
                    Rxj_log.append(log10(abs(Rxi_sz * (1 / teta_sz_iplus1) * dt)))
                ...
                if ci1 == 0:
                    cj_i1_log.append(ci1)
                else:
                    cj_i1_log.append(log10(abs(ci1)))

        # Corrective step

        if hpipe > 0.03:

            Mstor_ast = sum(c_usz_list[t]) * Ab * tteta_usz[t] * thusz[t] * 1 / m_usz + sum(c_sz_list[t]) * Ab * tteta_sz[
                t] * thsz[t] * 1 / m_sz
            ...
            MRx = - (sum(Rx_usz_list[t]) * Ab * tteta_usz[t] * thusz[t] * 1 / m_usz + sum(Rx_sz_list[t]) * Ab * tteta_sz[
                t] * thsz[t] * 1 / m_sz)
        ...
        if hpipe == 0:
            ...
            Mstor_mb = M_in_acum[-1] - M_pz_list[-1] - M_over_acum[-1] - Mpipe_acum[-1] - Minf_sz_acum[-1] - M_et_acum[-1] - \
                       Msoil_acum[-1] - MRx_acum[-1]
            Mstor_mb_list.append(Mstor_mb)
        ...
        #### salvar os valores em log ####
        lista_log_c_usz_list.append(cl_i1_log)
        lista_log_csoil_usz_list.append((csoil_i_usz_log))
        lista_log_Rx_usz_list.append(Rxl_log)
        ...
        if hpipe >= 0.03:
            ...
            # salvar valores em log
            lista_log_c_sz_list.append(cj_i1_log)
            lista_log_csoil_sz_list.append(csoil_i_sz_log)
            lista_log_Rx_sz_list.append(Rxj_log)
    ...
    # data_EColi = pd.concat((frames), axis=1)
      ...
      # **5. Transforming in dataframe LOG **
      ## E Coli

      # print('lista_log_c_usz_list', lista_log_c_usz_list)
      # print('len', len(lista_log_c_usz_list))
      # input()


    log_data_usz = pd.DataFrame(lista_log_c_usz_list)
    a = 0
    column_name = []
    for i in range(m_usz):
        name = 'usz' + str(a)
        column_name.append(name)
        a = a + 1
    log_data_usz.set_axis(column_name, axis='columns', inplace=True)

    log_data_sz = pd.DataFrame(lista_log_c_sz_list)
    a = 0
    column_name = []
    for i in range(m_sz):
        name = 'sz' + str(a)
        column_name.append(name)
        a = a + 1
    log_data_sz.set_axis(column_name, axis='columns', inplace=True)

    # print('LOG_data_usz', log_data_usz)
    # print('LOG_data_sz', log_data_sz)
    # input()

    # print('lista_log_csoil_usz_list', lista_log_csoil_usz_list)
    # print('len', len(lista_log_csoil_usz_list))
    # input()
    log_data_soil_usz = pd.DataFrame(lista_log_csoil_usz_list)
    a = 0
    column_name = []
    for i in range(m_usz):
        name = 'usz_soil' + str(a)
        column_name.append(name)
        a = a + 1
    log_data_soil_usz.set_axis(column_name, axis='columns', inplace=True)

    # print('LOG_data_soil_usz', log_data_soil_usz)


    log_data_soil_sz = pd.DataFrame(lista_log_csoil_sz_list)
    a = 0
    column_name = []
    for i in range(m_sz):
        name = 'sz_soil' + str(a)
        column_name.append(name)
        a = a + 1
    log_data_soil_sz.set_axis(column_name, axis='columns', inplace=True)

    # print('LOG_data_soil_sz', log_data_soil_sz)
    # input()

    log_data_rx_usz = pd.DataFrame(lista_log_Rx_usz_list)
    a = 0
    column_name = []
    for i in range(m_usz):
        name = 'usz_rx' + str(a)
        column_name.append(name)
        a = a + 1
    log_data_rx_usz.set_axis(column_name, axis='columns', inplace=True)

    log_data_rx_sz = pd.DataFrame(lista_log_Rx_sz_list)
    a = 0
    column_name = []
    for i in range(m_sz):
        name = 'sz_rx' + str(a)
        column_name.append(name)
        a = a + 1
    log_data_rx_sz.set_axis(column_name, axis='columns', inplace=True)

    frames2 = [log_data_usz, log_data_sz, log_data_soil_usz, log_data_soil_sz, log_data_rx_usz, log_data_rx_sz]

    data_EColi_log = pd.concat((frames2), axis=1)
  # lista_log_cpz.append(0)

    cpz_list_calib = lista_log_cpz[:]
    cpz_list_calib.append(0)
    data_EColi_log['lista_log_cpz'] = lista_log_cpz
    #print(len(indice))
    #input()

    #indice_list = indice[:]

    indice_n = list(range(len(lista_log_cpz)))


    # print(len(lista_log_cpz))
    # input()


    #print(len(log_cin_list))
    #input()


    log_cin_list.append(0)
    c_in_log = log_cin_list[:len(indice_n)]

    #c_in_log = log_cin_list[:len(indice)]

    data_EColi_log['c_in_log'] = c_in_log


    # cin = log_cin_list[:]
    # cin.append(0)
    # c_in_log = cin[:len(indice_list)]
    #
    # data_EColi_log['c_in_log'] = c_in_log

    # data_EColi_log['t'] = indice_list
    #
    # Qorif_list = tQpipe[:len(indice_list)]


    data_EColi_log['t'] = indice_n

    Qorif_list = tQpipe[:len(indice_n)]


    data_EColi_log['Qorif'] = Qorif_list

    if hpipe > 0:
        data_EColi_log['Morif'] = data_EColi_log['Qorif'] * data_EColi_log['sz3'] * 1000  # m3/s
    else:
        data_EColi_log['Morif'] = data_EColi_log['Qorif'] * data_EColi_log['usz10'] * 1000  # m3/s
    ...
    # data_EColi_log['Qorif'] = Qorif_list
    # #print(tQpipe)
    # #print(len(Qorif_list))
    #
    #
    # if hpipe > 0:
    #     data_EColi_log['Morif'] = data_EColi['Qorif'] * data_EColi['sz3']*1000   # m3/s
    # else:
    #     data_EColi_log['Morif'] = data_EColi['Qorif'] * data_EColi['usz10']*1000 # m3/s
